...
use tree_sitter_c2rust::{Language, Query};
...
pub fn use_query<T>(cx: Scope<T>, query: &str) -> Signal<Query> {
    ...
    let signal = use_signal(cx, || Query::new(*language(), query).unwrap());

    use_effect(cx, &query.to_owned(), |q| async move {
        signal.set(Query::new(*language(), &q).unwrap())
    });

    let query = query.to_owned();
    dioxus_signals::use_effect(cx, move || {
        signal.set(Query::new(*language(), &query).unwrap())
    });
